        ...
        for eval_file in recent_files:
            try:
                ...
                if log.status == "success" and log.samples:
                    ...
                    existing_tasks = {getattr(l.eval, 'task', None) for l in eval_logs}
...
def run_sweep_experiment(
    ...
    for model in model_names:
        ...
        if data_model not in datasets["base_models"]:
            if data_model != model:
                print(f"Warning: Data for '{model}' (COT-I, uses '{data_model}') not found in {dataset_dir_path}")
            else:
    ...
    for model_name in model_names:
        ...
        control_path = f"data/input/{dataset_name}/{data_subset}/{data_model_name}/data.json"
    ...
    if batch and no_batch_tasks:
        ...
        if no_batch_tasks:
            ...
            try:
                ...
                _collect_partial_results(shared_log_dir, eval_logs, no_batch_descriptions)
    ...
    for idx, eval_log in enumerate(eval_logs):
        ...
        description = descriptions[idx] if idx < len(descriptions) else f"Evaluation {idx}"
