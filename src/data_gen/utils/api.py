...
class BatchAPIClient(ABC):
    ...
    @abstractmethod
    def prepare_request_format(
        self, custom_id: str, content: str, config: dict, model: str
    ) -> dict:
        """
...
class OpenAIBatchClient(BatchAPIClient):
    ...
    def create_batch(
        self, upload_id: str, model: str, requests: Optional[List[dict]] = None
    ) -> Dict[str, Any]:
...
class AnthropicBatchClient(BatchAPIClient):
    ...
    def create_batch(
        self, upload_id: None, model: str, requests: List[dict]
    ) -> Dict[str, Any]:
        if not requests:
...
class FireworksBatchClient(BatchAPIClient):
    ...
    def upload_requests(self, request_data_path: Path, model: str) -> str:
        ...
        create_resp = self.session.post(
            create_url, json={"datasetId": dataset_id, "dataset": {"userUploaded": {}}}
        )
...
class TogetherBatchClient(BatchAPIClient):
    ...
    def upload_requests(self, request_data_path: Path, model: str) -> str:
        file_resp = self.client.files.upload(
            file=str(request_data_path), purpose="batch-api"
        )
        return file_resp.id
    ...
    def create_batch(
        ...
        batch = self.client.batches.create_batch(
            upload_id, endpoint="/v1/chat/completions"
        )
        return {
            "batch_id": batch.id,
            "status": batch.status,
...
def get_api_client(api_name: str) -> BatchAPIClient:
    ...
    if api_name not in clients:
        raise ValueError(
            f"Unknown API: {api_name}. Must be one of {list(clients.keys())}"
        )
