...
def check_and_download_batch(
    api_client, metadata: dict, raw_path: Path, generation_dir: Path
) -> bool:
    """
...
def transform_to_output(
    raw_path: Path, output_path: Path, metadata: dict, api_name: str
):
...
def main():
    ...
    if metadata.get("received"):
        ...
        if output_file.exists():
            ...
            print(
                "Warning: received=True but output file not found. Re-transforming..."
            )
            raw_path = generation_dir / "raw.jsonl"
            if raw_path.exists():
                transform_to_output(
                    raw_path, output_file, metadata, metadata["api_name"]
                )
            else:
    ...
    print("Updated request status: received=True")
