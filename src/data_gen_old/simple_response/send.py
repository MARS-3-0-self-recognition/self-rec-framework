...
def load_config(config_path: Path) -> dict:
    ...
    if config["output_type"] not in ["summaries", "answers"]:
        raise ValueError(
            f"output_type must be 'summaries' or 'answers', got {config['output_type']}"
        )
...
def create_request_data(
    content_data: dict, config: dict, model_path: str, api_client
) -> tuple[list, dict]:
    """
    ...
    for content_uuid, content_text in content_data.items():
        ...
        request = api_client.prepare_request_format(
            custom_id=custom_id, content=content_text, config=config, model=model_path
        )
...
def main():
    ...
    parser.add_argument(
        "--model", required=True, help="Model in format {api}/{model_path}"
    )
    parser.add_argument(
        "--data", required=True, help="Dataset name (directory under data/)"
    )
    parser.add_argument(
        "--generation_config", required=True, help="Path to generation config YAML"
    )
    ...
    generation_name = (
        config_path.stem
    )  # e.g., "simple_config" from "simple_config.yaml"
    ...
    requests, custom_id_map = create_request_data(
        content_data, config, model_path, api_client
    )
    ...
    print("Run receive script to check status and download results.")
